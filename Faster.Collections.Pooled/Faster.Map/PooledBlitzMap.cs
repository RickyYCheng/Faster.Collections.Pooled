...
using Faster.Collections.Pooled.Contracts;

namespace Faster.Collections.Pooled;

// By RickyYC:
// The Alloc/Free methods are virtual to allow overriding in derived classes
// to implement custom memory management strategies
public partial class PooledBlitzMap<TKey, TValue, THasher>
{
    [Pure]
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    protected virtual Bucket[] AllocBuckets(int size)
    {
        return GC.AllocateUninitializedArray<Bucket>(size);   
    }

    [Pure]
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    protected virtual Entry[] AllocEntries(int size)
    {
        return GC.AllocateUninitializedArray<Entry>(size);
    }

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    protected virtual void FreeBuckets(Bucket[]? buckets)
    {
        
    }

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    protected virtual void FreeEntries(Entry[]? entries)
    {
...
/// </typeparam>
public abstract partial class PooledBlitzMap<TKey, TValue, THasher> where THasher : struct, IHasher<TKey>
{
    #region Properties
    ...
    protected uint _mask;
    private uint _last;
    private const byte quadraticProbeLength = 6;
    protected static readonly byte INACTIVE = 0;
    protected int _length;
    private double _loadFactor;
    private uint _maxCountBeforeResize;
    protected THasher _hasher;
    protected uint _lastBucket = INACTIVE;

    #endregion

    /// <summary>
    /// Initializes a new instance of the <see cref="PooledBlitzMap{TKey, TValue, THasher}"/> class
    ...
    /// and performance.
    /// </remarks>
    public PooledBlitzMap() : this(2, 0.8) { }
    ...
    /// </remarks>
    public PooledBlitzMap(int length) : this(length, 0.8) { }
    ...
    public PooledBlitzMap(int length, double loadFactor)
    {
        ...
        FreeBuckets(_buckets);
        _buckets = AllocBuckets((int)cap);
        ...
        FreeEntries(_entries);
        _entries = AllocEntries((int)entryCap);
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public void Copy(PooledBlitzMap<TKey, TValue, THasher> other)
    {
    ...
    private void Resize()
    {
        ...
        _entries = AllocEntries(entryCount);
        _buckets = AllocBuckets(bucketCount);
        ...
        FreeEntries(oldEntriesArr);
        FreeBuckets(oldBucketsArr);
    }
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    protected uint EraseBucket(ref Bucket buckets, uint bucket, uint main)
    {
        ref Bucket victim = ref Unsafe.Add(ref buckets, bucket);
        uint next = victim.Next;
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    protected void EraseSlot(ref Bucket buckets, ref Entry entries, uint slot, uint ebucket)
    {
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    protected uint SigToBucket(ref Bucket buckets, ref Entry entries, uint index)
    {
