...
internal static partial class EnumerableHelpers
{
    ...
    internal static T[] ToPooledArray<T>(IEnumerable<T> source, out int length, ArrayPool<T>? pool = null)
    {
        // Copied from Array.MaxLength in System.Private.CoreLib/src/libraries/System.Private.CoreLib/src/System/Array.cs
        const int ArrayMaxLength = 0X7FFFFFC7;
#if NET
                ...
                if (en.MoveNext())
                {
                    ...
                    while (en.MoveNext())
                    {
                        if (count == arr.Length)
                        {
                            ...
                            T[] newArr = pool.Rent(newLength);
                            Array.Copy(arr, newArr, count);
                            pool.Return(arr, clearArray: RuntimeHelpers.IsReferenceOrContainsReferences<T>());
                            arr = newArr;
                        }
