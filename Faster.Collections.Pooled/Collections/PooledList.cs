...
[Serializable]
public partial class PooledList<T> : IList<T>, IList, IReadOnlyList<T>, IDisposable
{
...
#pragma warning restore CA1825, IDE0300
    ...
    object? IList.this[int index]
    {
        ...
        set
        {
            ...
            {
                throw new ArgumentException($"Value is of type '{value?.GetType().Name}' and cannot be converted to type '{typeof(T).Name}'.", nameof(value));
            }
        }
    }
    ...
    int IList.Add(object? item)
    {
        ...
        {
            throw new ArgumentException($"Item is of type '{item?.GetType().Name}' and cannot be converted to type '{typeof(T).Name}'.", nameof(item));
        }
    ...
    public int FindIndex(int startIndex, int count, Predicate<T> match)
    {
        ...
        {
            throw new ArgumentOutOfRangeException(nameof(startIndex), "Start index must be less than or equal to Count.");
        }
    ...
    public int FindLastIndex(int startIndex, int count, Predicate<T> match)
    {
        ...
        if (_size == 0)
        {
            ...
            if (startIndex != -1)
            {
                throw new ArgumentOutOfRangeException(nameof(startIndex), "Start index must be less than Count.");
            }
        }
        else
        {
    ...
    /// </summary>
    /// <param name="start">The zero-based <see cref="PooledList{T}" /> index at which the range starts.</param>
    ...
    /// </exception>
    /// <exception cref="ArgumentException"><paramref name="start" /> and <paramref name="length" /> do not denote a valid range of elements in the <see cref="PooledList{T}" />.</exception>
    public PooledList<T> Slice(int start, int length) => GetRange(start, length);
    ...
    {
        if (index > _size)
            throw new ArgumentOutOfRangeException(nameof(index), "Index must be less than or equal to Count.");
    ...
    public int LastIndexOf(T item, int index, int count)
    {
        ...
        if (index >= _size)
        {
            throw new ArgumentOutOfRangeException(nameof(index), "Index is larger than maximum index.");
        }

        if (count > index + 1)
        {
            throw new ArgumentOutOfRangeException(nameof(count), "Count is larger than maximum count.");
        }
