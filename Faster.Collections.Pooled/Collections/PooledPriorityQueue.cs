...
public partial class PooledPriorityQueue<TElement, TPriority>: IDisposable
{
    /// <summary>
    ...
    protected virtual (TElement Element, TPriority Priority)[] Alloc(int size)
    {
        return SingleThreadedArrayPool<(TElement, TPriority)>.Shared.Rent(size);
    }

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    protected virtual void Free((TElement Element, TPriority Priority)[] array)
    {
        if (array.Length != 0)
            SingleThreadedArrayPool<(TElement, TPriority)>.Shared.Return(array, RuntimeHelpers.IsReferenceOrContainsReferences<(TElement, TPriority)>());
    }

#if DEBUG
    static PooledPriorityQueue()
    {
    ...
    /// <summary>
    ///  Initializes a new instance of the <see cref="PooledPriorityQueue{TElement, TPriority}"/> class.
    /// </summary>
    public PooledPriorityQueue()
    {
    ...
    /// </exception>
    public PooledPriorityQueue(int initialCapacity)
    ...
    /// </param>
    public PooledPriorityQueue(IComparer<TPriority>? comparer)
    {
        _nodes = Array.Empty<(TElement, TPriority)>();
    ...
    ///  The specified <paramref name="initialCapacity"/> was negative.
    /// </exception>
    public PooledPriorityQueue(int initialCapacity, IComparer<TPriority>? comparer)
    {
    ...
    /// </remarks>
    public PooledPriorityQueue(IEnumerable<(TElement Element, TPriority Priority)> items)
    ...
    ///  which is generally faster than enqueuing individual elements sequentially.
    /// </remarks>
    public PooledPriorityQueue(IEnumerable<(TElement Element, TPriority Priority)> items, IComparer<TPriority>? comparer)
    {
        ArgumentNullException.ThrowIfNull(items);

        var pooled = EnumerableHelpers.ToPooledArray(items, out _size);
        _nodes = Alloc(_size);
        Array.Copy(pooled, _nodes, _size);
        if (pooled.Length != 0)
            ArrayPool<(TElement, TPriority)>.Shared.Return(pooled, clearArray: RuntimeHelpers.IsReferenceOrContainsReferences<(TElement, TPriority)>());
    ...
    /// <summary>
    ///  Gets the number of elements contained in the <see cref="PooledPriorityQueue{TElement, TPriority}"/>.
    /// </summary>
    ...
    /// <summary>
    ///  Gets the priority comparer used by the <see cref="PooledPriorityQueue{TElement, TPriority}"/>.
    /// </summary>
    ...
    /// <summary>
    ///  Adds the specified element with associated priority to the <see cref="PooledPriorityQueue{TElement, TPriority}"/>.
    /// </summary>
    /// <param name="element">The element to add to the <see cref="PooledPriorityQueue{TElement, TPriority}"/>.</param>
    ...
    /// <summary>
    ///  Returns the minimal element from the <see cref="PooledPriorityQueue{TElement, TPriority}"/> without removing it.
    /// </summary>
    /// <exception cref="InvalidOperationException">The <see cref="PooledPriorityQueue{TElement, TPriority}"/> is empty.</exception>
    /// <returns>The minimal element of the <see cref="PooledPriorityQueue{TElement, TPriority}"/>.</returns>
    public TElement Peek()
    {
        if (_size == 0)
        {
            throw new InvalidOperationException($"Priority queue {this} is empty.");
        }
    ...
    /// <summary>
    ///  Removes and returns the minimal element from the <see cref="PooledPriorityQueue{TElement, TPriority}"/>.
    /// </summary>
    ...
    /// <summary>
    ///  Removes the minimal element and then immediately adds the specified element with associated priority to the <see cref="PooledPriorityQueue{TElement, TPriority}"/>,
    /// </summary>
    /// <param name="element">The element to add to the <see cref="PooledPriorityQueue{TElement, TPriority}"/>.</param>
    /// <param name="priority">The priority with which to associate the new element.</param>
    /// <exception cref="InvalidOperationException">The queue is empty.</exception>
    ...
    /// <summary>
    ///  Removes the minimal element from the <see cref="PooledPriorityQueue{TElement, TPriority}"/>,
    ...
    ///  <see langword="false"/> if the <see cref="PooledPriorityQueue{TElement, TPriority}"/> is empty.
    /// </returns>
    ...
    /// <summary>
    ///  Returns a value that indicates whether there is a minimal element in the <see cref="PooledPriorityQueue{TElement, TPriority}"/>,
    ///  and if one is present, copies it to the <paramref name="element"/> parameter,
    ///  and its associated priority to the <paramref name="priority"/> parameter.
    ///  The element is not removed from the <see cref="PooledPriorityQueue{TElement, TPriority}"/>.
    /// </summary>
    ...
    /// <summary>
    ///  Enqueues a sequence of element/priority pairs to the <see cref="PooledPriorityQueue{TElement, TPriority}"/>.
    /// </summary>
    ...
    /// <summary>
    ///  Enqueues a sequence of elements pairs to the <see cref="PooledPriorityQueue{TElement, TPriority}"/>,
    ...
    /// <summary>
    ///  Removes all items from the <see cref="PooledPriorityQueue{TElement, TPriority}"/>.
    /// </summary>
    public void Clear()
    {
    ...
    /// <summary>
    ///  Ensures that the <see cref="PooledPriorityQueue{TElement, TPriority}"/> can hold up to
    ...
    /// </exception>
    /// <returns>The current capacity of the <see cref="PooledPriorityQueue{TElement, TPriority}"/>.</returns>
    ...
    /// <summary>
    ///  Sets the capacity to the actual number of items in the <see cref="PooledPriorityQueue{TElement, TPriority}"/>,
    ...
    public void TrimExcess()
    {
        ...
        if (_size < threshold)
        {
            ...
            Array.Copy(_nodes, newArray, _size);
            Free(_nodes);
            _nodes = newArray;
            _version++;
        }
    ...
    /// <summary>
    ///  Enumerates the contents of a <see cref="PooledPriorityQueue{TElement, TPriority}"/>, without any ordering guarantees.
    /// </summary>
    [DebuggerDisplay("Count = {Count}")]
    [DebuggerTypeProxy(typeof(PriorityQueueDebugView<,>))]
    public sealed class UnorderedItemsCollection : IReadOnlyCollection<(TElement Element, TPriority Priority)>, ICollection
    {
        internal readonly PooledPriorityQueue<TElement, TPriority> _queue;

        internal UnorderedItemsCollection(PooledPriorityQueue<TElement, TPriority> queue) => _queue = queue;
        ...
        void ICollection.CopyTo(Array array, int index)
        {
            ...
            {
                throw new ArgumentOutOfRangeException(nameof(index), index, nameof(index));
            }
        ...
        /// <summary>
        ///  Enumerates the element and priority pairs of a <see cref="PooledPriorityQueue{TElement, TPriority}"/>,
        ///  without any ordering guarantees.
        /// </summary>
        public struct Enumerator : IEnumerator<(TElement Element, TPriority Priority)>
        {
            private readonly PooledPriorityQueue<TElement, TPriority> _queue;
            private readonly int _version;
            private int _index;
            private (TElement, TPriority) _current;

            internal Enumerator(PooledPriorityQueue<TElement, TPriority> queue)
            {
                _queue = queue;
                _index = 0;
            ...
            {
                PooledPriorityQueue<TElement, TPriority> localQueue = _queue;
